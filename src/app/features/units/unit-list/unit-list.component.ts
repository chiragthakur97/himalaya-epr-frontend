...
export class UnitListComponent implements OnInit {
  ...
  readonly columns: TableColumn<Unit>[] = [
  ...
  onAction(event: { action: string; row: Unit }): void {
    if (event.action === 'edit') {
      this.router.navigate(['/units', event.row.id, 'edit']);
  ...
  deleteUnit(unit: Unit): void {
    this.dialog
      .open(DeleteDialogComponent, {
        data: {
          title: 'Delete Unit',
          message: 'Are you sure you want to delete this unit? Products using it cannot be deleted.',
          itemName: unit.name,
          confirmLabel: 'Delete',
        },
      })
